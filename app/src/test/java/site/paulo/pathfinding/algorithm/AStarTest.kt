...
internal class AStarTest {
    ...
    fun `test A* algorithm in grid graph - reverse`() {
        val gridGraph = GridGraph(3,3)
        val aStart = AStar(gridGraph, gridGraph.getNode(Pair(2,2))!!, gridGraph.getNode(Pair(0,0))!!)
        aStart.run()
    ...
    fun `test A* algorithm in grid graph - no path - start at middle`() {
        ...
        val aStar = AStar(gridGraph, gridGraph.getNode(Pair(0,0))!!, gridGraph.getNode(Pair(5,5))!!)
